...
localizacion_ocupada = probpl.VariableDeEstados(
    nombre='localizacion_ocupada({l},{sn})',
    l=Localizaciones,
    sn=SiNO
)

localizacion_robot = probpl.VariableDeEstados(
    nombre='localizacion_robot({r},{l})',
    r=Robots,
    l=Localizaciones
)
...
def variableEstadoToString(listax):
    lista_modificada = []
    for x in listax:
...
lista_final = variableEstadoToString(lista)
print(lista_final)
...
for x in problema_estibadores.acciones:
    ...
    pre = variableEstadoToString(pre)
    efectn = variableEstadoToString(efectn)
    efectp = variableEstadoToString(efectp)

    lista_acciones.append(probpl.AccionesStrings(pre, efectp, efectn))
